...
### 9.5 Проверено
- Policy **Public**: прав доступа к коллекциям нет, доступ к приложению выключен, доступ администратора выключен.
- Policy **Administrator**: доступ к приложению включен, доступ администратора включен.
- Policy **api-access**: доступ к приложению включен, доступ администратора включен.
- Policy **scheduler-read**: доступ к приложению включен, доступ администратора выключен.

### 9.6 api-access / scheduler-read (по последним скринам)
- В обеих политиках отображается сообщение: **"У администраторов есть все права"**.
- Явных прав по коллекциям не задано (список разрешений пустой).
- **api-access**: доступ к приложению включен, доступ администратора включен.
- **scheduler-read**: доступ к приложению включен, доступ администратора включен (по скрину).
- Есть расхождение с предыдущей вкладкой (где `scheduler-read` показывался без админ‑доступа) — нужно перепроверить.

### 9.7 scheduler-read (скрин: права по коллекциям)
- Для пользовательских коллекций **arenas/bookings/clients/games** включены **create/read/update/delete/share** (полный доступ).
- Для системных коллекций (`directus_*`) включены **read** и местами **create/update/delete**.
- Это избыточно для роли “только чтение расписания”, требуется ограничить до **read** только нужных коллекций и отключить доступ к системным.

### 9.8 scheduler-read после исправления
- Пользовательские коллекции **arenas/bookings/clients/games**: только **read**.
- Системные коллекции (`directus_*`): доступ отключён.

### 9.9 api-access (скрин: текущие права)
- Пользовательские коллекции:
  - **arenas**: read
  - **bookings**: create / read / update / delete
  - **clients**: read
  - **games**: read
- Системные коллекции (`directus_*`): есть read, местами create/update/delete (избыточно).
- Рекомендуется оставить только необходимые user‑коллекции и убрать доступ к `directus_*`.

### 9.10 api-access после ограничения
- Оставлены только необходимые user‑коллекции (arenas/games/clients/bookings).
- Доступ к `directus_*` отключён.
